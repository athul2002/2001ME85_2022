...
def main():
    ...
    page_title="Octant Analyser",
     ...
     if choice=="Single File":
        ...
        clicked = st.button('Folder Picker',on_click=state)
        if "ste" not in st.session_state:
            st.session_state.ste=False
        if clicked or st.session_state.ste:
            mod=st.number_input("Enter mod value",min_value=0)

            # st.session_state.load_state=True
            dirname = st.text_input('Selected folder:', filedialog.askdirectory(master=root))
            files = [file for file in os.listdir(dirname) if not file.startswith('.')] # Ignore hidden files
            x=st.button("Compute")
            ...
            if x :
                ...
                # for data_files in files:
                    # data_file.append(data_files)
                
                    # st.write(type(data_file))
                for data_file in files:    
                    df=pd.read_excel(dirname+"/"+data_file)
                    data_file= st.runtime.uploaded_file_manager.UploadedFile(data_file)
                    function2(df,mod,data_file)
                colouring(files,len(df),mod)
                zipfile(files,mod)
                download2()
                os.remove("test.zip")
                for data_file in data_files:
