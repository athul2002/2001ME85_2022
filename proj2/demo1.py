...
# def dropping(data_file):
#     for datafile in data_file:
#         wb = openpyxl.load_workbook(datafile.name[:-5]+"converted.xlsx")
#         ws = wb.active
...
#                 s = ws.cell(r,c).value
#                 if s != None and "nan" in s: 
#                     ws.cell(r,c).value = s.replace("nan"," ") 

#                     # print("row {} col {} : {}".format(r,c,s))
#                     i += 1

        # wb.save(datafile.name[:-5]+"converted.xlsx")
def modification(df,datafile):
    writer = pd.ExcelWriter(datafile.name[:-5]+"converted.xlsx", engine='xlsxwriter')
    df.to_excel(writer, index=False, sheet_name='Sheet1')
    writer.save()
    wb = load_workbook(datafile.name[:-5]+'converted.xlsx')
    sheet = wb.active
    # sheet.number_format='General'
    ...
    for cell in sheet['AW1':'AY' + str(17 + sum([sheet['AU' + str(i)].value for i in range(2, 10)]))]:
        for c in cell:
            c.border = border
    wb.save(filename=datafile.name[:-5]+"converted.xlsx")  
def colouring(data_file):
    for datafile in data_file:
        wb = load_workbook(datafile.name[:-5]+'converted.xlsx')
        sheet = wb.active
        # # giving colour for rank1 ocant
        # style = DifferentialStyle(fill=PatternFill(bgColor='FFFF00'))
        # # style = PatternFill(fill_type=None,start_color='FFFF00',end_color='FFFF00')
        # rank_rule = Rule(type="containsText", dxf=style)
...
def download1(data_files):
    for datafile in data_files:
        with open(datafile.name[:-5]+"converted.xlsx", 'rb') as my_file:
            st.download_button(label = 'Download'+datafile.name[:-5], data = my_file, file_name =datafile.name[:-5]+"converted.xlsx", mime = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
def zipfile(data_file):
    zip_path = "test.zip"
    # my_zipfile.close()
    with ZipFile("test.zip","w") as zip:
        for datafile in data_file:
            zip.write(datafile.name[:-5]+"converted.xlsx")
...
#prining the overall count and rank of octants
    ...
    for x,octval in enumerate([1,-1,2,-2,3,-3,4,-4]):
        ...
        st.write(type(df.at[2+len(range_list)+x,'Rank Octant 4']))
###################################################################################################################
###################################################################################################################
...
#                   printing the longest subsequence with time range        
    ...
    st.write(df)
    modification(df,data_file)
    # df_modi = pd.read_excel(data_file[:-5]+"converted.xlsx")
    # df_xlsx=to_excel(df_modi)
...
def main():
     ...
     if choice=="Single File":
        ...
        data_files=st.file_uploader("Upload csv",type=["xlsx"],accept_multiple_files=True)
        if data_files is not None:
            mod=st.number_input("Enter number",min_value=5)
            x=st.button("Compute")
            if(x):
                ...
                # download(data_files)
                # dropping(data_files)
                colouring(data_files)
                # zipfile(data_files)
                download1(data_files)

                for data_file in data_files:
                    os.remove(data_file.name[:-5]+"converted.xlsx")
        ...
        if data_files is not None:
            ...
            if(x):
                ...
                for data_file in data_files:
                    os.remove(data_file.name[:-5]+"converted.xlsx")
            #    mod=st.number_input("Enter a number")
